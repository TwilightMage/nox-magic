...
struct FSQLTypes {
	...
	FString Downcast(FString &Input) {
...
USTRUCT(meta = (RestrictedToClasses = SQLite, BlueprintInternalUseOnly = true))
struct USQLITE_API FSQLTableColumn {
	GENERATED_USTRUCT_BODY()
	//
	//
	UPROPERTY(Category = "USQL", EditDefaultsOnly)
...
struct USQLITE_API FSQLTable {
	...
	//
	UPROPERTY(Category = "USQL", EditDefaultsOnly)
	TArray<FSQLTableColumn> Columns;
	//
	UPROPERTY(Category="USQL", EditDefaultsOnly)
	bool Lock;
};
...
FORCEINLINE FArchive &operator << (FArchive &Ar, FSQLTableColumn &UST) {
	Ar << UST.Type;
	Ar << UST.Name;
	Ar << UST.NN;
	Ar << UST.PK;
...
public:
	...
	UFUNCTION()
	static FString BuildFieldTypeName(FString typeName, bool NN, bool PK, bool AI, bool U);

	UFUNCTION()
	static int32 FindColumn(TArray<FSQLTableColumn> arr, FString columnName);
};
