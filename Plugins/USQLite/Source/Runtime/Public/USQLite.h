...
protected:
	...
	FString DB_SelectCondition;
	//
	/** SQL Command Condition Statement: DELETE. */
	UPROPERTY(Category="USQL", EditDefaultsOnly, BlueprintReadOnly, AdvancedDisplay, meta=(DisplayName="[DATABASE] DELETE Condition"))
	FString DB_DeleteCondition;
	//
	/** Enables Debug Logs. */
	...
	virtual FString DB_CreateTableSQL(FString tableName = "");

	FString DB_CopyFromOldSQL(FString tableName = "");
	//
...
#if WITH_EDITORONLY_DATA
	void OnTableDirty();
	void OnRenameColumn(FString from, FString to);
	void OnAddColumn(FString name, FString type);
