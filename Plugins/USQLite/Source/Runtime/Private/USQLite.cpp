...
USQLite::USQLite() {
	...
	if (DB_TABLE.Columns.Num()==0) {
		FSQLTableColumn newColumn;
		newColumn.Type = USQL::USQLTypes.Ints;
		newColumn.Name = "ID";
		newColumn.NN = true;
		newColumn.PK = true;
		newColumn.AI = true;
		...
		DB_TABLE.Columns.Add(newColumn);
	}///
	//
...
void USQLite::DB_Setup() {
	...
	//
	if (DB_SelectCondition.IsEmpty()) {DB_SelectCondition = TEXT("FROM {Table} WHERE {Key} = {ID}");}
	if (DB_DeleteCondition.IsEmpty()) {DB_DeleteCondition = TEXT("FROM {Table} WHERE {Key} = {ID}");}
	//
	USQL::DB_Execute(DB_RUNTIME, DB_FILE, DB_CreateTableSQL());
	//
	for (auto VS : DB_VERSIONS)
	{
		USQL::DB_Execute(DB_RUNTIME, DB_FILE, DB_CreateTableSQL(GetNameSanitized() + "_" + VS));
	}///
#endif
	//
	DBS_QUEUE.Empty();
...
void USQLite::DB_RefreshPreview() {
	...
	if (DB_DataPreview.Rows.Num()==0) {
		FSQLRow NoData;
		 ...
		 for (const auto& column : DB_TABLE.Columns) NoData.PreviewTypes.Add(column.Name);
...
FString USQLite::DB_CreateTableSQL(FString tableName)
{
	if (tableName.IsEmpty()) tableName = GetNameSanitized();
	...
	{
		columnTypes.Add(FString::Printf(TEXT("`%s` %s"), *DB_TABLE.Columns[I].Name, *USQL::BuildFieldTypeName(USQL::USQLTypes.Downcast(DB_TABLE.Columns[I].Type), DB_TABLE.Columns[I].NN, DB_TABLE.Columns[I].PK, DB_TABLE.Columns[I].AI, DB_TABLE.Columns[I].U)));
	}
	SQL = FString::Printf(TEXT("CREATE TABLE IF NOT EXISTS `%s` (%s);"), *tableName, *FString::Join(columnTypes, *FString(", ")));
#endif
	return SQL;
}

FString USQLite::DB_CopyFromOldSQL(FString tableName)
{
	if (tableName.IsEmpty()) tableName = GetNameSanitized();
	...
	for (int32 I = 0; I < DB_TABLE.Columns.Num(); ++I)
	{
		columnNames.Add("`" + DB_TABLE.Columns[I].Name + "`");
	}
	SQL = FString::Printf(TEXT("INSERT INTO `%s` (%s) SELECT %s FROM `%s_old`"), *tableName, *FString::Join(columnNames, *FString(", ")), *FString::Join(columnNames, *FString(", ")), *GetNameSanitized());
#endif
	return SQL;
}
...
FString USQLite::DB_GenerateSQL_Object_INSERT(UObject* Object) {
	...
	//
	for (int32 I=0; I<DB_TABLE.Columns.Num(); ++I) {
		Header += FString::Printf(TEXT("`%s`,"),*DB_TABLE.Columns[I].Name);
	...
	for (int32 I=1; I<DB_TABLE.Columns.Num(); ++I) {
		...
		for (TFieldIterator<UProperty>PIT(Object->GetClass(),EFieldIteratorFlags::IncludeSuper); PIT; ++PIT) {
			...
			auto ColumnID = DB_GetMappedColumnID(Object->GetClass(),DB_TABLE.Columns[I].Name);
			...
			//
			if ((IsSet)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.Sets)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<USetProperty>(Property),Object)+TEXT(","); continue;}
			if ((IsMap)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.Maps)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UMapProperty>(Property),Object)+TEXT(","); continue;}
			if ((IsInt)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.Ints)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UIntProperty>(Property),Object)+TEXT(","); continue;}
			if ((IsBool)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.Bools)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UBoolProperty>(Property),Object)+TEXT(","); continue;}
			if ((IsByte)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.Bytes)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UByteProperty>(Property),Object)+TEXT(","); continue;}
			if ((IsEnum)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.Enums)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UEnumProperty>(Property),Object)+TEXT(","); continue;}
			if ((IsName)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.Names)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UNameProperty>(Property),Object)+TEXT(","); continue;}
			if ((IsText)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.Texts)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UTextProperty>(Property),Object)+TEXT(","); continue;}
			if ((IsArray)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.Arrays)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UArrayProperty>(Property),Object)+TEXT(","); continue;}
			if ((IsFloat)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.Floats)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UFloatProperty>(Property),Object)+TEXT(","); continue;}
			if ((IsString)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.Strings)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UStrProperty>(Property),Object,ESQLWriteMode::String)+TEXT(","); continue;}
			if ((IsStruct)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.Colors)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UStructProperty>(Property),Object,ESQLWriteMode::Color)+TEXT(","); continue;}
			if ((IsStruct)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.Structs)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UStructProperty>(Property),Object,ESQLWriteMode::Struct)+TEXT(","); continue;}
			if ((IsStruct)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.Rotators)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UStructProperty>(Property),Object,ESQLWriteMode::Rotator)+TEXT(","); continue;}
			if ((IsStruct)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.Vector2D)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UStructProperty>(Property),Object,ESQLWriteMode::Vector2D)+TEXT(","); continue;}
			if ((IsStruct)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.Vector3D)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UStructProperty>(Property),Object,ESQLWriteMode::Vector3D)+TEXT(","); continue;}
			if ((IsString)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.TimeStamps)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UStrProperty>(Property),Object,ESQLWriteMode::TimeStamp)+TEXT(","); continue;}
			if ((IsStruct)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.DateTimes)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UStructProperty>(Property),Object,ESQLWriteMode::DateTime)+TEXT(","); continue;}
			if ((IsObject)&&(DB_TABLE.Columns[I].Type==USQL::USQLTypes.ObjectPtrs)) {Values.RemoveFromEnd(DBNULL); Values += USQL::UPropertyToSQL(CastChecked<UObjectProperty>(Property),Object,ESQLWriteMode::ObjectPtr)+TEXT(","); continue;}
		Property = nullptr;}
	}///
	//
	Values += FString(TEXT(");"));
...
FString USQLite::DB_GenerateSQL_Object_SELECT(UObject* Object) {
	...
	if (ID.IsEmpty()) {return TEXT("ERROR:NOID");}
	//
	FString SQL = TEXT("SELECT ");
	FString CDT = DB_SelectCondition;
	//
	for (int32 I=0; I<DB_TABLE.Columns.Num(); ++I) {
		SQL += (DB_TABLE.Columns[I].Name+TEXT(", "));
	}///
	//
	CDT = CDT.Replace(TEXT("{ID}"),*ID);
	CDT = CDT.Replace(TEXT("{Key}"),*DB_TABLE.Columns[0].Name);
	//
	if (DB_VERSION.CurrentVersion.IsEmpty()) {CDT = CDT.Replace(TEXT("{Table}"),*GetNameSanitized());}
	else {CDT = CDT.Replace(TEXT("{Table}"),*((GetNameSanitized()+TEXT("_"))+DB_VERSION.CurrentVersion));}
	//
	SQL += CDT+TEXT(";");
...
FString USQLite::DB_GenerateSQL_Object_DELETE(UObject* Object) {
	...
	//
	FString SQL = TEXT("DELETE ");
	FString CDT = DB_DeleteCondition;
	//
	CDT = CDT.Replace(TEXT("{ID}"),*ID);
	CDT = CDT.Replace(TEXT("{Key}"),*DB_TABLE.Columns[0].Name);
	//
	...
	if (NET != NM_Client) {
		for (TObjectIterator<UObject>Object; Object; ++Object) {
			...
			if ((*Object)->IsA(AActor::StaticClass())) {
				...
				if (Actor->GetLocalRole() != ROLE_Authority) {continue;}
			} else {
				const auto &Owner = (*Object)->GetTypedOuter<AActor>();
				if (Owner) {
					if (Owner->GetLocalRole() != ROLE_Authority) {continue;}
				}///
			}///
			//
			const auto &Interface = Cast<ISQLiteData>(*Object);
...
ESQLResult USQLite::DB_UPDATE_Boolean(const FString &RowID, const FString &ColumnName, const bool Value) {
#if WITH_SERVER_CODE
	const int32 ID = USQL::FindColumn(DB_TABLE.Columns, ColumnName);
	if ((ID==INDEX_NONE)||(!DB_TABLE.Columns.IsValidIndex(ID))) {
	...
	//
	if (!DB_TABLE.Columns[ID].Type.Equals(USQL::USQLTypes.Bools)) {
	...
	SQL = Header+Values+FString::Printf(TEXT("WHERE `%s` = '%s'"),*DB_TABLE.Columns[0].Name,*RowID);
	//
	LOG_DB(Debug,ESQLSeverity::Info,SQL);
	return USQL::DB_Execute(DB_RUNTIME,DB_FILE,SQL);
#endif
...
ESQLResult USQLite::DB_UPDATE_Byte(const FString &RowID, const FString &ColumnName, const uint8 Value) {
	...
	//
	if (!DB_TABLE.Columns[ID].Type.Equals(USQL::USQLTypes.Bytes)) {
...
ESQLResult USQLite::DB_UPDATE_Integer(const FString &RowID, const FString &ColumnName, const int32 Value) {
	...
	//
	if (!DB_TABLE.Columns[ID].Type.Equals(USQL::USQLTypes.Ints)) {
...
ESQLResult USQLite::DB_UPDATE_Float(const FString &RowID, const FString &ColumnName, const float Value) {
	...
	//
	if (!DB_TABLE.Columns[ID].Type.Equals(USQL::USQLTypes.Floats)) {
...
ESQLResult USQLite::DB_UPDATE_Name(const FString &RowID, const FString &ColumnName, const FName Value) {
	...
	//
	if (!DB_TABLE.Columns[ID].Type.Equals(USQL::USQLTypes.Names)) {
...
ESQLResult USQLite::DB_UPDATE_Text(const FString &RowID, const FString &ColumnName, const FText Value) {
	...
	//
	if (!DB_TABLE.Columns[ID].Type.Equals(USQL::USQLTypes.Texts)) {
...
ESQLResult USQLite::DB_UPDATE_String(const FString &RowID, const FString &ColumnName, const FString Value) {
	...
	//
	if (!DB_TABLE.Columns[ID].Type.Equals(USQL::USQLTypes.Strings)) {
...
ESQLResult USQLite::DB_UPDATE_Date(const FString &RowID, const FString &ColumnName, const FDateTime Value) {
	...
	//
	if (!DB_TABLE.Columns[ID].Type.Equals(USQL::USQLTypes.DateTimes)) {
...
ESQLResult USQLite::DB_UPDATE_Color(const FString &RowID, const FString &ColumnName, const FColor Value) {
	...
	//
	if (!DB_TABLE.Columns[ID].Type.Equals(USQL::USQLTypes.Colors)) {
...
ESQLResult USQLite::DB_UPDATE_Vector2D(const FString &RowID, const FString &ColumnName, const FVector2D Value) {
	...
	//
	if (!DB_TABLE.Columns[ID].Type.Equals(USQL::USQLTypes.Vector2D)) {
...
ESQLResult USQLite::DB_UPDATE_Vector3D(const FString &RowID, const FString &ColumnName, const FVector Value) {
	...
	//
	if (!DB_TABLE.Columns[ID].Type.Equals(USQL::USQLTypes.Vector3D)) {
...
ESQLResult USQLite::DB_UPDATE_Rotator(const FString &RowID, const FString &ColumnName, const FRotator Value) {
	...
	//
	if (!DB_TABLE.Columns[ID].Type.Equals(USQL::USQLTypes.Rotators)) {
...
ESQLResult USQLite::DB_UPDATE_Object(const FString &RowID, const FString &ColumnName, const UObject* OBJ) {
	...
	//
	if (!DB_TABLE.Columns[ID].Type.Equals(USQL::USQLTypes.ObjectPtrs)) {
	...
	if ((OBJ!=nullptr)&&OBJ->IsValidLowLevelFast()) {
		...
		if (OBJ->IsA(AActor::StaticClass())) {
			FullName = USQLReflector::MakeActorDBID(const_cast<AActor*>(CastChecked<AActor>(OBJ)));
		} else if (OBJ->IsA(UActorComponent::StaticClass())) {
			FullName = USQLReflector::MakeComponentDBID(const_cast<UActorComponent*>(CastChecked<UActorComponent>(OBJ)));
		} else {FullName=USQLReflector::MakeObjectDBID(const_cast<UObject*>(OBJ));}
		//
		auto ClassPath = OBJ->GetClass()->GetDefaultObject()->GetPathName();
		//
		const FString Target = (ClassPath+TEXT("::")+FullName);
...
ESQLResult USQLite::DB_UPDATE_Boolean_Array(const FString &RowID, const FString &ColumnName, const TArray<bool>&Values) {
	...
	//
	if (!DB_TABLE.Columns[ID].Type.Equals(USQL::USQLTypes.Arrays)) {
	...
	SQL = Header+Body+FString::Printf(TEXT("WHERE `%s` = '%s'"),*DB_TABLE.Columns[0].Name,*RowID);
	//
	LOG_DB(Debug,ESQLSeverity::Info,SQL);
	return USQL::DB_Execute(DB_RUNTIME,DB_FILE,SQL);
#endif
...
ESQLResult USQLite::DB_UPDATE_Object_Array(const FString &RowID, const FString &ColumnName, const TArray<UObject*>&Values) {
	...
	for (int32 I=0, N=Values.Num()-1; I<=N; I++) {
		...
		if ((OBJ!=nullptr)&&OBJ->IsValidLowLevelFast()) {
			FString FullName;
			if (OBJ->IsA(AActor::StaticClass())) {
				FullName = USQLReflector::MakeActorDBID(CastChecked<AActor>(OBJ));
			} else if (OBJ->IsA(UActorComponent::StaticClass())) {
				FullName = USQLReflector::MakeComponentDBID(CastChecked<UActorComponent>(OBJ));
			} else {FullName=USQLReflector::MakeObjectDBID(OBJ);}
			//
			auto ClassPath = OBJ->GetClass()->GetDefaultObject()->GetPathName();
			//
			const FString Target = (ClassPath+TEXT("::")+FullName);
...
FSQLRow USQLite::DB_SELECT_Row(const FString &RowID) {
#if WITH_SERVER_CODE
	if (!DB_TABLE.Columns.IsValidIndex(0)) {
	...
	//
	SQL = SQL+FString::Printf(TEXT(" WHERE `%s` = '%s'"),*DB_TABLE.Columns[0].Name,*RowID);
	//
	//
	FSQLRow Row;
	DBSTM Statement;
...
ESQLResult USQLite::DB_DELETE_Row(const FString &RowID) {
	...
	//
	FString SQL;
	if (DB_VERSION.CurrentVersion.IsEmpty()) {SQL=FString::Printf(TEXT("DELETE FROM %s WHERE `%s` = '%s'"),*GetNameSanitized(),*DB_TABLE.Columns[0].Name,*RowID);}
	else {SQL=FString::Printf(TEXT("DELETE FROM %s WHERE `%s` = '%s'"),*((GetNameSanitized()+TEXT("_"))+DB_VERSION.CurrentVersion),*DB_TABLE.Columns[0].Name,*RowID);}
	//
	//
	LOG_DB(Debug,ESQLSeverity::Info,SQL);
								...
								if (VAL.Cast==ESQLReadMode::ObjectPtr) {
									...
									if ((CDO!=nullptr)&&CDO->IsA(AActor::StaticClass())) {
										for (TActorIterator<AActor>Actor(Context->GetWorld()); Actor; ++Actor) {
											if (USQLReflector::MakeActorDBID(*Actor)==Instance) {return (*Actor); break;}
										}///
									} else if (CDO!=nullptr) {
										for (TObjectIterator<UObject>OBJ; OBJ; ++OBJ) {
											if (USQLReflector::MakeObjectDBID(*OBJ)==Instance) {return (*OBJ); break;}
										}///
									}///
									//
									return nullptr;
								...
								if (VAL.Cast==ESQLReadMode::Array) {
									...
									for (int32 I=0, N=JArray.Num()-1; I<=N; I++) {
										...
										if (JValue.IsValid()&&(!JValue->IsNull())) {
											...
											if ((CDO!=nullptr)&&CDO->IsA(AActor::StaticClass())) {
												for (TActorIterator<AActor>Actor(Context->GetWorld()); Actor; ++Actor) {
													if (USQLReflector::MakeActorDBID(*Actor)==Instance) {Data.Add(*Actor); break;}
												}///
											} else if (CDO!=nullptr) {
												for (TObjectIterator<UObject>OBJ; OBJ; ++OBJ) {
													if (USQLReflector::MakeObjectDBID(*OBJ)==Instance) {Data.Add(*OBJ); break;}
												}///
											}///
										}///
									}///
...
TArray<FName> USQLite::DB_SELECT_IDs() {
	...
	//
	FString SQL;
	if (DB_VERSION.CurrentVersion.IsEmpty()) {SQL=FString::Printf(TEXT("SELECT %s FROM %s"),*DB_TABLE.Columns[0].Name,*GetNameSanitized());}
	else {SQL = FString::Printf(TEXT("SELECT %s FROM %s"),*DB_TABLE.Columns[0].Name,*(GetNameSanitized()+TEXT("_")+DB_VERSION.CurrentVersion));}
	//
	//
	DBSTM Statement;
	TArray<FName>Data;
				...
				if (Row.Values.Num()>0) {
					for (const auto &VAL : Row.Values) {
						switch (VAL.Type) {
							...
							//
							default:
								LOG_DB(Debug,ESQLSeverity::Warning,FString::Printf(TEXT("[%s] (SELECT ALL IDs Command): Target Column Type mismatch; Column is not a Name Field: %s"),*GetName(),*DB_TABLE.Columns[0].Name));
						break;}///
					}///
				}///
			} else {LOG_DB(true,ESQLSeverity::Error,FString(ANSI_TO_TCHAR(ErrorMSG)));}
						...
						switch (VAL.Type) {
							case ESQLType::Text:
							{
								...
								if (VAL.Cast==ESQLReadMode::ObjectPtr) {
									...
									if ((CDO!=nullptr)&&CDO->IsA(AActor::StaticClass())) {
										for (TActorIterator<AActor>Actor(Context->GetWorld()); Actor; ++Actor) {
											if (USQLReflector::MakeActorDBID(*Actor)==Instance) {Data.Add(*Actor); continue;}
										}///
									} else if (CDO!=nullptr) {
										for (TObjectIterator<UObject>OBJ; OBJ; ++OBJ) {
											if (USQLReflector::MakeObjectDBID(*OBJ)==Instance) {Data.Add(*OBJ); continue;}
										}///
									}///
								}///
							}	break;
...
void USQLite::OnTableDirty()
{
	...
	{
		targetTable = GetNameSanitized();
	}
	else
	{
		targetTable = (GetNameSanitized() + TEXT("_")) + DB_VERSION.CurrentVersion;
	}
	USQL::DB_Execute(DB_RUNTIME, DB_FILE, FString::Printf(TEXT("ALTER TABLE `%s` RENAME TO `%s_old`"), *targetTable, *targetTable));
	USQL::DB_Execute(DB_RUNTIME, DB_FILE, DB_CreateTableSQL(targetTable));
	if (USQL::DB_Execute(DB_RUNTIME, DB_FILE, DB_CopyFromOldSQL(targetTable)) == ESQLResult::OK)
	{
		USQL::DB_Execute(DB_RUNTIME, DB_FILE, FString::Printf(TEXT("DROP TABLE IF EXISTS `%s_old`"), *targetTable));
	}
	else
	{
		USQL::DB_Execute(DB_RUNTIME, DB_FILE, FString::Printf(TEXT("DROP TABLE IF EXISTS `%s`"), *targetTable));
		USQL::DB_Execute(DB_RUNTIME, DB_FILE, FString::Printf(TEXT("ALTER TABLE `%s_old` RENAME TO `%s`"), *targetTable, *targetTable));
	}
	//
	DBS_QUEUE.Empty();
	DBL_QUEUE.Empty();
...
void USQLite::OnRenameColumn(FString from, FString to)
{
	if (DB_VERSION.CurrentVersion.IsEmpty())
	{
		USQL::DB_Execute(DB_RUNTIME, DB_FILE, FString::Printf(TEXT("ALTER TABLE `%s` RENAME `%s` TO `%s`"), *GetNameSanitized(), *from, *to));
	}
	else
	{
		USQL::DB_Execute(DB_RUNTIME, DB_FILE, FString::Printf(TEXT("ALTER TABLE `%s` RENAME `%s` TO `%s`"), *((GetNameSanitized() + TEXT("_")) + DB_VERSION.CurrentVersion), *from, *to));
	}
...
void USQLite::OnAddColumn(FString name, FString type)
{
	if (DB_VERSION.CurrentVersion.IsEmpty())
	{
		USQL::DB_Execute(DB_RUNTIME, DB_FILE, FString::Printf(TEXT("ALTER TABLE `%s` ADD COLUMN `%s` %s"), *GetNameSanitized(), *name, *type));
	}
	else
	{
		USQL::DB_Execute(DB_RUNTIME, DB_FILE, FString::Printf(TEXT("ALTER TABLE `%s` ADD COLUMN `%s` TO %s"), *((GetNameSanitized() + TEXT("_")) + DB_VERSION.CurrentVersion), *name, *type));
	}
...
void USQLite::OnPreviewDelete(const FString RowID) {
	...
	FString SQL;
	if (DB_VERSION.CurrentVersion.IsEmpty()) {
		SQL = FString::Printf(TEXT("DELETE FROM `%s` WHERE `%s` = '%s'"),*GetNameSanitized(),*DB_TABLE.Columns[0].Name,*RowID);
	} else {SQL = FString::Printf(TEXT("DELETE FROM `%s` WHERE `%s` = '%s'"),*((GetNameSanitized()+TEXT("_"))+DB_VERSION.CurrentVersion),*DB_TABLE.Columns[0].Name,*RowID);}
	//
...
void USQLite::OnPreviewPaste(const FString FromID, const FString ToID) {
	...
	for (int32 I=1; I<DB_TABLE.Columns.Num(); ++I) {
		if (DB_VERSION.CurrentVersion.IsEmpty()) {
			SQL += FString::Printf(TEXT("UPDATE `%s` SET `%s` = (SELECT `%s` FROM `%s` WHERE `%s` = '%s') WHERE `%s` = '%s'"),*GetNameSanitized(),*DB_TABLE.Columns[I].Name,*DB_TABLE.Columns[I].Name,*GetNameSanitized(),*DB_TABLE.Columns[0].Name,*FromID,*DB_TABLE.Columns[0].Name,*ToID)+TEXT("; ");
		} else {
			SQL += FString::Printf(TEXT("UPDATE `%s` SET `%s` = (SELECT `%s` FROM `%s` WHERE `%s` = '%s') WHERE `%s` = '%s'"),*((GetNameSanitized()+TEXT("_"))+DB_VERSION.CurrentVersion),*DB_TABLE.Columns[I].Name,*DB_TABLE.Columns[I].Name,*((GetNameSanitized()+TEXT("_"))+DB_VERSION.CurrentVersion),*DB_TABLE.Columns[0].Name,*FromID,*DB_TABLE.Columns[0].Name,*ToID)+TEXT("; ");
		}///
	}///
	//
	auto Result = USQL::DB_Execute(DB_RUNTIME,DB_FILE,SQL);
