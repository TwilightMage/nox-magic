...
ESQLResult USQL::DB_Execute(sqlite3* DB, const FString File, const FString Query) {
	...
	if (DB_OpenDatabase(DB,File)) {
		...
		LOG_DB(true, ESQLSeverity::Info, FString::Printf(TEXT("[%d (%s)] %s"), Result, *UEnum::GetValueAsString<ESQLResult>(Result).RightChop(12), *Query));
		DBT_END(DB,ErrorMSG);
		//
...
bool USQL::GetPlayerNetworkID(const APlayerController* PlayerController, FString &ID, const bool AppendPort) {
	return USQLReflector::GetPlayerNetworkID(PlayerController,ID,AppendPort);
}

FString USQL::BuildFieldTypeName(FString typeName, bool NN, bool PK, bool AI, bool U)
{
	FString result = typeName;
	if (NN) result += " NOT NULL";
	if (PK) result += " PRIMARY KEY";
	if (AI) result += " AUTOINCREMENT";
	if (U) result += " UNIQUE";

	return result;
}

int32 USQL::FindColumn(TArray<FSQLTableColumn> arr, FString columnName)
{
	for (int32 i = 0; i < arr.Num(); i++)
	{
		if (arr[i].Name == columnName) return i;
	}
...
FString USQL::MakeObjectDBID(UObject* OBJ) {
	return USQLReflector::MakeObjectDBID(OBJ);
}

FString USQL::MakeActorDBID(AActor* Actor) {
	return USQLReflector::MakeActorDBID(Actor);
}

FString USQL::MakeComponentDBID(UActorComponent* CMP) {
	return USQLReflector::MakeComponentDBID(CMP);
}
