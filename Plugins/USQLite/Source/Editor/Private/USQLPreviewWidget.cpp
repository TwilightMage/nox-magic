...
void SQLPreviewWidget::Construct(const FArguments &InArgs) {
	...
	PreviewGrid = SNew(SUniformGridPanel).MinDesiredSlotWidth(35);
...
FReply SQLPreviewWidget::OnClickedUPSERT() {
	...
	if (Database->DB_TABLE.Columns.Num()!=NewDataBoxes.Num()) {return FReply::Handled();}
	//
	//
	...
	for (int32 I=0; I<Database->DB_TABLE.Columns.Num(); ++I) {
		Header += FString::Printf(TEXT("%s,"),*Database->DB_TABLE.Columns[I].Name);
	} Header += TEXT(") ");
	//
	...
	for (TSharedPtr<SEditableTextBox> Field : NewDataBoxes) {
		...
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.Bools)) {
		...
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.Bytes)) {
			const uint8 Value = FCString::Atoi(*Field->GetText().ToString());
			Values += FString::Printf(TEXT("%i"),Value)+TEXT(",");
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.Ints)) {
			const int32 Value = FCString::Atoi(*Field->GetText().ToString());
			Values += FString::Printf(TEXT("%i"),Value)+TEXT(",");
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.Floats)) {
			const float Value = FCString::Atof(*Field->GetText().ToString());
			Values += FString::Printf(TEXT("%f"),Value)+TEXT(",");
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.Enums)) {
			Values += (FString(TEXT("'UEnum>>"))+Field->GetText().ToString()+TEXT("',"));
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.Names)) {
			Values += (FString(TEXT("'UName>>"))+Field->GetText().ToString()+TEXT("',"));
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.Texts)) {
			Values += (FString(TEXT("'UText>>"))+Field->GetText().ToString()+TEXT("',"));
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.Strings)) {
			Values += (FString(TEXT("'UString>>"))+Field->GetText().ToString()+TEXT("',"));
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.TimeStamps)) {
			Values += FString(TEXT("DATETIME('now','localtime'),"));
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.DateTimes)) {
			const int64 Value = FCString::Atoi64(*Field->GetText().ToString());
			Values += FString::Printf(TEXT("%i"),Value)+TEXT(",");
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.Arrays)) {
			Values += (FString(TEXT("'UArray>>{"))+Field->GetText().ToString()+TEXT("}',"));
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.Sets)) {
			Values += (FString(TEXT("'USet>>{"))+Field->GetText().ToString()+TEXT("}',"));
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.Maps)) {
			Values += (FString(TEXT("'UMap>>{"))+Field->GetText().ToString()+TEXT("}',"));
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.Colors)) {
			Values += (FString(TEXT("'UColor>>{"))+Field->GetText().ToString()+TEXT("}',"));
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.Vector2D)) {
			Values += (FString(TEXT("'UVector2D>>{"))+Field->GetText().ToString()+TEXT("}',"));
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.Vector3D)) {
			Values += (FString(TEXT("'UVector3D>>{"))+Field->GetText().ToString()+TEXT("}',"));
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.Rotators)) {
			Values += (FString(TEXT("'URotator>>{"))+Field->GetText().ToString()+TEXT("}',"));
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.Structs)) {
			Values += (FString(TEXT("'UStruct>>{"))+Field->GetText().ToString()+TEXT("}',"));
		V++; continue;}
		//
		if (Database->DB_TABLE.Columns[V].Type.Equals(USQL::USQLTypes.ObjectPtrs)) {
