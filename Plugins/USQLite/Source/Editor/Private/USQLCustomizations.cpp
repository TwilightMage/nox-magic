...
void FUSQLDetails::CustomizeDetails(IDetailLayoutBuilder &DetailBuilder) {
	...
	[
		SNew(SUniformGridPanel).MinDesiredSlotWidth(100).SlotPadding(FMargin(2.0f))
		+SUniformGridPanel::Slot(0,0).HAlign(HAlign_Fill)
		[
			SNew(SButton).IsEnabled(this,&FUSQLDetails::OnEnable)
		...
		[
			...
			[
				...
				.Justification(ETextJustify::Center)
				.Font(FEditorStyle::GetFontStyle(TEXT("BoldFont")))
				.Text(LOCTEXT("ClearTable","RESET COLUMNS"))
...
bool FUSQLDetails::OnEnable() const {
	return (USQL::GetThreadSafety()==ESQLThreadSafety::IsCurrentlyThreadSafe);
}
...
FReply FUSQLDetails::OnAddTableColumn(IDetailLayoutBuilder* DetailBuilder) {
	...
	if (USQLiteOBJ&&(!USQLiteOBJ->DB_TABLE.Lock)) {
		...
		newColumn.Type = USQL::USQLTypes.Texts;
		newColumn.Name = FString::Printf(TEXT("Field %i"), USQLiteOBJ->DB_TABLE.Columns.Num());
		newColumn.NN = false;
		newColumn.PK = false;
		newColumn.AI = false;
		newColumn.U = false;
		USQLiteOBJ->DB_TABLE.Columns.Add(newColumn);

		USQLiteOBJ->OnAddColumn(newColumn.Name, USQL::BuildFieldTypeName(USQL::USQLTypes.Downcast(newColumn.Type), newColumn.NN, newColumn.PK, newColumn.AI, newColumn.U));
		//
...
void FUSQLTableDetails::CustomizeHeader(TSharedRef<IPropertyHandle> StructPropertyHandle, FDetailWidgetRow &HeaderRow, IPropertyTypeCustomizationUtils &StructCustomizationUtils) {
	...
	[
		SNew(SQLTableWidget).Columns(Columns).Lock(Lock)
	];
}
